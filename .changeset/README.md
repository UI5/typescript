...
1. **In your PR**, run `pnpm changeset` and answer the prompts (which packages, semver level,
...
If a change does not warrant a release (docs-only, internal tooling, test refactors, etc.) run `pnpm changeset:empty` and commit the generated file — it satisfies the required changeset check without producing any package bump.
